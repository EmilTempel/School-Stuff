...
public class GUI {
	...
	public GUI() {
		...
		PointOfView PoV = new PointOfView(new Vector(-20, 0, 0), FOV, width);
		PoV.setAlpha(Math.toRadians(0));
		PoV.setBeta(Math.toRadians(0));
		PoV.setS(new Vector(0,0,0));
		
		double R = 3;
		double r = 1;
		
//		Mesh m = Mesh.MarchingCubes(new Vector(5,5,5), -10, 20, v -> v.x()*v.x()+v.y()*v.y()-3);
//		m.addMesh(Mesh.MarchingCubes(new Vector(5,5,5), -10, 20, v -> Vector.dot(Vector.sub(v,new Vector(-1,0,-3.5)), Vector.sub(v,new Vector(-1,0,0)))-r));
//		Mesh m = Mesh.MarchingCubes(new Vector(5,5,5), -10, 20, v -> 0);
//		Mesh m = Mesh.MarchingCubes(new Vector(8,8,8), -16, 20, v -> Math.pow((Vector.dot(v, v)+R*R-r*r),2)-4*R*R*(v.x()*v.x()+v.y()*v.y()) );
//		Mesh m =  Mesh.MarchingCubes(new Vector(5,5,5), -10, 20, v -> Math.tan(v.x(0))*Math.exp(v.x(1)) -v.x(2));
		Mesh m =  Mesh.MarchingCubes(new Vector(5,5,5), -10, 40, v -> Math.sin(v.x(0))*Math.cos(v.x(1))-v.x(2));
...
//		Mesh m = new Polytope(new Vector(-5,-5,-5),10,10,10).calcFaces();
		
		JLabel lbl = new JLabel() {
			Vector u = new Vector(0,0,1);
			Vector w = new Vector(-1,0,0);
			protected void paintComponent(Graphics g) {
//				PoV.setS(Vector.mult(Vector.norm(Vector.add(PoV.getS(), Vector.mult(Vector.norm(Vector.cross(u,w)),1))), 20));
